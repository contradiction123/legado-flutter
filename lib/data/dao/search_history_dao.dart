...
class SearchHistoryDao {
  ...
  Future<List<SearchKeyword>> getAll() async {
    final items = await (_database.select(_database.searchKeywords)
  ...
  Future<void> add(String word) async {
    ...
    final existing = await (_database.select(_database.searchKeywords)
          ..where((t) => t.word.equals(word)))
        .getSingleOrNull();

    if (existing != null) {
      await (_database.update(_database.searchKeywords)
            ..where((t) => t.word.equals(word)))
          .write(db.SearchKeywordsCompanion(
            usage: Value(existing.usage + 1),
            lastUseTime: Value(now),
          ));
    } else {
      await _database.into(_database.searchKeywords).insert(
  ...
  Future<int> delete(String word) {
    return (_database.delete(_database.searchKeywords)
          ..where((t) => t.word.equals(word)))
        .go();
  }
