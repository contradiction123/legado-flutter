...
class CacheDao {
  ...
  Future<Cache?> get(String key) async {
    final item = await (_database.select(_database.caches)
  ...
  Future<int> set(String key, String value, {int? deadline}) {
    return _database.into(_database.caches).insertOnConflictUpdate(
      db.CachesCompanion(
        key: Value(key),
        value: Value(value),
  ...
  Future<int> delete(String key) {
    return (_database.delete(_database.caches)
          ..where((t) => t.key.equals(key)))
        .go();
  }
