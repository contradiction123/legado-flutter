...
class ReplaceRuleDao {
  ...
  Future<List<ReplaceRule>> getEnabled() async {
    final items = await (_database.select(_database.replaceRules)
  ...
  Future<int> insert(ReplaceRule rule) {
    return _database.into(_database.replaceRules).insert(
  ...
  Future<bool> update(ReplaceRule rule) {
    return _database.update(_database.replaceRules).replace(
      _mapper.toCompanion(rule),
    );
  }
  ...
  Future<int> delete(int id) {
    return (_database.delete(_database.replaceRules)
