...
class HighlightRuleDao {
  ...
  Future<List<HighlightRule>> getEnabled() async {
    final items = await (_database.select(_database.highlightRules)
  ...
  Future<int> insert(HighlightRule rule) {
    return _database.into(_database.highlightRules).insert(
      _mapper.toCompanion(rule),
    );
  }
  ...
  Future<int> update(HighlightRule rule) {
    return (_database.update(_database.highlightRules)
        ...
        .write(_mapper.toCompanion(rule));
  }

  /// 根据 ID 删除高亮规则
  Future<int> delete(String id) {
    return (_database.delete(_database.highlightRules)
  ...
  Future<int> toggleEnabled(String id) async {
    final rule = await (_database.select(_database.highlightRules)
          ..where((t) => t.id.equals(id)))
        .getSingleOrNull();
    if (rule == null) return 0;
    return (_database.update(_database.highlightRules)
          ..where((t) => t.id.equals(id)))
        .write(db.HighlightRulesCompanion(
